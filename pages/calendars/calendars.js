...
Page({
  ...
  queryMothMemo:  function(this_month,days_style)  {
    return new Promise((resolve, reject) => {
      let match = new RegExp(this_month)
      query.queryData(db, 'memo', data => {
        // 使用箭头函数保持上下文this指向
      let list = data.memoList
      let date = Object.keys(list)
      date.map( d => {
          if (match.test(d) && list[d].lists.length){
            days_style.unshift({
              month: 'current',
  ...
  onLoad:  function(options) {
    ...
    this.setData({  date: today.date })
      // 获取Memo列表数据
    this.queryMothMemo(`${today.year}-${today.month}`,days_style).then( data => {
     if (data) {
       this.setData({
         id: data._id,
         memo: data.memoList,
